...
<template>
    ...
    >
      ...
      <div class="flex mb-2" :class="userStore.isSidebarCollapsed ? 'justify-center' : 'justify-end'">
        <button @click="userStore.toggleSidebar()" class="p-2 rounded-full hover:bg-white/10 transition-colors" title="收起/展开侧边栏">
      ...
      <!-- 歌单列表 -->
      <div class="bg-[#181818] rounded-lg flex-1 overflow-y-auto overflow-x-hidden">
        ...
        <ul v-if="userStore.profile" class="space-y-1 p-2">
          <li
            ...
            class="flex items-center p-2 cursor-pointer rounded-lg truncate transition-colors"
            :class="{ 
              'bg-blue-600 text-white': playerStore.currentPlaylist && playerStore.currentPlaylist.id === LIKED_SONGS_PLAYLIST_ID,
              'hover:bg-white/10': !(playerStore.currentPlaylist && playerStore.currentPlaylist.id === LIKED_SONGS_PLAYLIST_ID),
          ...
          <li v-for="playlist in userStore.playlists" :key="playlist.id"
              ...
              :class="{ 
                ...
                'hover:bg-white/10': !(playerStore.currentPlaylist && playerStore.currentPlaylist.id === playlist.id),
                'justify-center': userStore.isSidebarCollapsed
              }">
              <img 
